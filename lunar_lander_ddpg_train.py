...
class Actor(nn.Module): 
    ...
    def __init__(self):
        ...
        self.actor_target_network = MLP(8, 2, 64).to(device)
...
class Critic(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.critic_network = MLP(10, 1, 64).to(device)
        self.critic_target_network = MLP(10, 1, 64).to(device)
        self.critic_target_network.load_state_dict(self.critic_network.state_dict())
        self.discount_factor = 0.99
        self.alpha = 0.1
        self.critic_optimizer = torch.optim.Adam(self.critic_network.parameters(), lr=1e-3)
...
try:
    while total_steps < 10000:  
        ...
        while not (terminated or truncated):
            ...
            ddpg.replay_buffer_add(initial_state, action, obs, reward, terminated )
...
ddpg.save("ddpg_llc.pt", meta={'total_steps': total_steps,
                               'episode_rewards': episode_rewards})

# --- 📈 Plot the learning curve before eval ---
plt.figure(figsize=(8, 5))
plt.plot(episode_rewards, label="Episode reward", color="skyblue")
plt.plot(avg_rewards_smoothed, label=f"Moving average ({window})", color="orange")
plt.xlabel("Episode")
plt.ylabel("Total reward")
plt.title("DDPG Training Performance on LunarLanderContinuous-v2")
plt.legend()
